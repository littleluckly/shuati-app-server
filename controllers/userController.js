...
// 接口用途：忘记密码，发送验证码到用户邮箱（移动App重置密码流程）
// 使用场景：用户忘记密码时调用此接口，获取验证码后进行密码重置
// 参数说明：
// - email: 用户邮箱，必填
exports.forgotPassword = async (req, res, next) => {
  ...
  try {
    ...
    await db.collection("userActions").insertOne({
      ...
      email,
      verificationCode, // 存储验证码用于验证
      expiresAt,
      createdAt: now,
    });

    // 在实际应用中，这里应该发送邮件到用户邮箱，包含验证码
    console.log(
...
// 接口用途：重置密码
// 使用场景：用户在App中输入验证码后重置密码
// 参数说明：
// - email: 用户邮箱，必填
// - verificationCode: 验证码，必填（从邮件中获取）
// - newPassword: 新密码，必填
exports.resetPassword = async (req, res, next) => {
  ...
  if (!email || !verificationCode || !newPassword) {
    return res
      ...
      .json(
        ApiResponse.error(
          "缺少必要参数: email、verificationCode 和 newPassword"
        )
      );
  }
  ...
  try {
    ...
    const resetRequest = await db.collection("userActions").findOne({
      ...
      action: "reset_password_request",
      verificationCode,
      expiresAt: { $gt: now }, // 验证码未过期
      isUsed: { $ne: true }, // 验证码未被使用
    });
